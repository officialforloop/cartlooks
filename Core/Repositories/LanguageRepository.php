    ...
    public function changeStatus($id)
    {
        try {
            ...
            if ($language->status == 1 && ($language->code == 'en' || $language->code == getDefaultLang())) {
                toastNotification('error', 'You can not inactive this language', 'Failed');
            } else {
                $status = $language->status == 1 ? 2 : 1;
                $language->status = $status;
                $language->save();
                toastNotification('success', 'Language status updated successfully', 'Success');
            }
    ...
    public function deleteLanguage($id)
    {
        try {
            ...
            if ($code == 'en' || $code == getDefaultLang()) {
                toastNotification('error', 'Can not delete English and Default language', 'Failed');
            } else {
