    ...
    public function generateSitemap(Request $request): JsonResponse
    {
        try {
            ...
            if (isActivePlugin('cartlookscore')) {
                $products = \Plugin\CartLooksCore\Models\Product::where('status', config('settings.general_status.active'))
                    ->select('id', 'permalink')
                    ->get();
                $product_categories = \Plugin\CartLooksCore\Models\ProductCategory::where('status', config('settings.general_status.active'))
                    ->select('id', 'permalink')
                    ->get();
