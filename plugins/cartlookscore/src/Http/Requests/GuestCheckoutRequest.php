...
class GuestCheckoutRequest extends FormRequest
{
    ...
    public function rules(Request $request)
    {
        $rules = [
            ...
            'email' => 'nullable|email|unique:Plugin\CartLooksCore\Models\Customers,email,' . $request->id
        ];

        if (getEcommerceSetting('required_email_in_guest_checkout') == config('settings.general_status.active')) {
            $rules['email'] = 'nullable|email|unique:Plugin\CartLooksCore\Models\Customers,email,' . $request->id;
        }
