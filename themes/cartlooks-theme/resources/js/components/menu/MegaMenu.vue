<template>
  ...
  <div class="mega-menu-wrapper" ref="catDropdown">
    ...
    <button
      v-if="buttonStyleTwo"
      class="btn w-100 text-left text-uppercase d-flex align-items-center bg-black text-color-white font-weight-medium"
      :class="{ 'c4-bg': isShow }"
    ...
    <button
      v-else
      class="bg-transparent p-0 border-0 d-flex align-items-center custom-menu text-color-white font-weight-medium"
      @click="toggleMegaMenu"
    >
      <base-icon-svg class="mr-15" name="category" :height="5.5" :width="16" />
      {{ $t("Select Categories") }}
